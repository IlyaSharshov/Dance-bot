...
Провайдер видео: fal.ai (Seedance). Ключ: https://fal.ai/dashboard/keys
"""
...
async def on_generate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.info("on_generate: задача создана, status_url=%s", status_url)
...
async def fal_wait(status_url: str) -> str:
    ...
    while loop.time() < deadline:
        ...
        log.info("fal_wait: статус=%s", status)
        if status == "COMPLETED":
