...
PIAPI_TASK_TYPE = os.environ.get("PIAPI_TASK_TYPE", "seedance-2")
...
TEMPLATES = {
    ...
    "vibe_trip": {
        ...
        "prompt": (
            ...
            "Scene 1: the man from image 1 dances energetically and goofs around at a roadside gas station "
            "and truck stop parking lot, truck trailers in the background, handheld phone camera. "
            "Scene 2: the man from image 2 films himself selfie-style, smiling and waving at the camera, "
            "warm golden hour sunset light at the same gas station, the dancing man visible in the background. "
            "Scene 3: close-up of the woman from image 3 inside a car, wind blows through her hair, "
...
async def piapi_wait(task_id: str) -> str:
    ...
    while loop.time() < deadline:
        ...
        if status in ("failed", "error", "cancelled"):
            raise RuntimeError(f"Генерация не удалась: {json.dumps(data, ensure_ascii=False)[:400]}")
